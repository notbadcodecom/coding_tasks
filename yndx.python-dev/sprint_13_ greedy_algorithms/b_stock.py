...
# Оно находится в диапазоне от 0 до 10000.
# # Во второй строке через пробел записаны n целых чисел в диапазоне
# от 0 до 1000.
#
# Формат вывода
...
def stock_count(n, history, count=0):
    for i in range(n):
        if history[i] < history[i + 1]:
            count += (history[i + 1] - history[i])
    return count

n = int(input())
history = [int(x) for x in input().split(' ')] + [0]
print(stock_count(n, history))
